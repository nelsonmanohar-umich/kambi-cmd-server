...
parser.add_argument('pwd', type=str, help='your linux pwd from which api request is being made within client machine')
parser.add_argument('user', type=str, help='your linux username within client machine')
...
async def request_handler_function(reqid, cmdstr, ip, cmd, options, path):
    ...
    if valid:
        ...
        if not path.strip() or os.path.exists(path) or glob.glob(path):
            ...
            # thread = Thread(target=threaded_task, args=(reqid, cmd, options, path))
            # thread.start()
...
class EntryPoint(Resource):
    ...
    def get(self):
        ...
        ServerCache.reqids[reqid] = t_now
        ...
        ret = self.request_handler(reqid, ip, cmd, options, path)
        return ret
